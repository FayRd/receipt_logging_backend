...
│ Structured JSON      │  │ GET  /devices/me     │  │ POST /chat/query     │
└──────────────────────┘  │ POST /devices/link   │  │ Gemini 3.6 Flash RAG │
                          └──────────────────────┘  └──────────────────────┘
```
...
| 4 | `/api/v1/devices/link` | `POST` | `X-Device-ID`, `X-Device-Token` | Links device to user account & migrates orphan guest data | **Implemented** |
...
| 7 | `/api/v1/user/me` | `GET` | `X-Device-ID`, `X-Device-Token` | Retrieves authenticated user profile (requires user session) | **Implemented** |
| 8 | `/api/v1/scan/parse` | `POST` | Multipart upload | Extracts structured JSON via Gemini 3.6 Flash Vision API | **Implemented** |
| 9 | `/api/v1/receipts/` | `GET` | `X-Device-ID`, `X-Device-Token` | Gets all non-deleted receipts owned by session identity | **Implemented** |
| 10 | `/api/v1/receipts/{receipt_id}` | `GET` | `X-Device-ID`, `X-Device-Token` | Gets single receipt by UUID (session ownership enforced) | **Implemented** |
| 11 | `/api/v1/receipts/` | `POST` | `X-Device-ID`, `X-Device-Token` | Creates a receipt record bound to session identity | **Implemented** |
| 12 | `/api/v1/receipts/batch` | `POST` | `X-Device-ID`, `X-Device-Token` | Batch creates up to 100 receipts bound to session identity | **Implemented** |
| 13 | `/api/v1/receipts/{receipt_id}` | `DELETE` | `X-Device-ID`, `X-Device-Token` | Soft-deletes receipt by UUID (session ownership enforced) | **Implemented** |
| 14 | `/api/v1/chat/create` | `POST` | `X-Device-ID`, `X-Device-Token` | Creates new AI conversation (max 10 limit per identity) | **Implemented** |
| 15 | `/api/v1/chat/list` | `GET` | `X-Device-ID`, `X-Device-Token` | Lists conversations owned by session identity | **Implemented** |
| 16 | `/api/v1/chat/history` | `GET` | `X-Device-ID`, `X-Device-Token` | Gets conversation message history (chunked limits) | **Implemented** |
| 17 | `/api/v1/chat/query` | `POST` | `X-Device-ID`, `X-Device-Token` | Conversational AI query over receipt history with RAG | **Implemented** |

---

## Guest-to-User Data Migration & Privacy Architecture

- **On Link (`POST /api/v1/devices/link` with `user_id`)**:
  - Automatically migrates all existing guest receipts and conversations (`device_id = X AND user_id IS NULL`) by updating their `user_id` to the newly linked user UUID.
- **On Unlink / Logout (`POST /api/v1/devices/link` with `user_id = null`)**:
  - Updates `devices.user_id = NULL`.
  - Previously created receipts and conversations **remain securely stamped** with `user_id = UserA`.
  - Guest sessions on that device query `user_id IS NULL` and see **0 receipts** (complete privacy isolation).
...
- **Testing & Quality Assurance**: Automated Pytest suite (37 tests in `test/`) with `test-engineer` subagent and `security-advisor` vulnerability auditor
