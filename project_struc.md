...
For full project scope and API specifications, see [PROJECT_SCOPE.md](PROJECT_SCOPE.md).
...
│   │       ├── devices.py      # Device registration & session-scoped device routes
...
│   │   └── identity.py         # Identity Pydantic model, get_current_identity, require_user_identity
...
│   │   │   ├── __init__.py     # Package marker
│   │   │   └── device_repository.py # Device DB CRUD & token validation logic
...
2. **Authentication & Identity Resolution (`src/Auth/`)**:
   ...
   - Performs constant-time cryptographic verification (`secrets.compare_digest`) against the stored `device_token`.
   - Returns a verified `Identity` object that automatically scopes downstream queries.
...
4. **Models & Repositories (`src/Models/`)**:
   - Defines strict Pydantic models (`Receipt`, `LineItem`, `UserRecord`, `DeviceRecord`).
   - Grouped into model-specific repository directories (`Receipts`, `Users`, `Devices`), encapsulating all direct Supabase `AsyncClient` queries, identity filtering, soft deletes (`deleted_at`), and batch insertions.
