...
2. Use the provided Receipt Context data to give specific answers (dates, amounts, merchants, categories).
3. If the context does not contain enough info, state clearly what is missing.
4. Provide numeric summaries or line-item breakdowns when requested.
""".strip()
...
class ChatService:
    ...
    async def generate_response(
        ...
        2. Builds a structured context block listing date, merchant, category, amount.
        ...
        # 2. Format receipt context string
        context_lines = []
        for r in recent_receipts:
            rec_data = r.get("receipt", {})
            merchant = rec_data.get("merchant_name", "Unknown")
            total = rec_data.get("total_amount", 0.0)
            curr = rec_data.get("currency", "USD")
            cat = rec_data.get("category", "General")
            date_str = str(rec_data.get("date", ""))[:10]
            context_lines.append(f"- [{date_str}] {merchant} ({cat}): {curr} {total}")
        
        context_block = "User's Recent Receipts:\n" + (
        ...
        for m in history_messages[-10:]:  # last 10 turns for context window
            ...
            formatted_contents.append(
                types.Part.from_text(text=f"{role_prefix}{m['content']}")
            )
        formatted_contents.append(types.Part.from_text(text=f"User: {user_message}"))
